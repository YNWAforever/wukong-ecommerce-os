...
2. Deploy compatible Web and Worker readers **before any production `reliable_source` authority records are written**. Their canonical subject name is the lower-case domain. Old readers reject this additive enum. Authenticated append-only registry writes require a separate reviewed operator action; local implementation authorizes no production seeding.
3. Obtain concrete approval for production additive migrations and each Worker/Vercel publication. Keep flags false through schema/readiness checks and mixed legacy/wine draining verification. Verify exact build SHA, Hyperdrive/Queue connectivity, document callback, HTTPS image snapshots, R2 conditional writes, namespace permissions and retention against the real topology.
