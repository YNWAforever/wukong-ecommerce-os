...
Task 7A (36020e6) adds workspace-scoped count/page queries for catalog, listings and all five Jobs ledger sources. Catalog filtering no longer truncates at 5,000 rows; quality scans active versions in bounded batches instead of stopping at 100. API responses disclose scope, missing/unassessable versions and quality scan start/end times. Jobs separates all-history ledger scope from the 30-day metrics window. Counts and IDs use a SQL snapshot; subsequent detail hydration and advisory readiness may observe concurrent changes.
