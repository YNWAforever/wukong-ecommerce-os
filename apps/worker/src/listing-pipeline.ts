...
export type PipelineRepositories = {
  ...
  pipelineRuns: {
    ...
    claimStep(input: { idempotencyKey: string; listingId: string; activeVersionSequence: number; step: PipelineStepName }): Promise<{ claimed: boolean; completed: boolean; output: unknown }>;
    recordStep(input: { idempotencyKey: string; listingId?: string; activeVersionSequence?: number; step: PipelineStepName; output?: unknown }): Promise<void>;
    complete(input: { idempotencyKey: string; listingId: string; activeVersionSequence: number; status: PipelineResult["status"]; versionId: string | null }): Promise<void>;
    fail(input: { idempotencyKey: string; listingId: string; activeVersionSequence: number; errorCode: PipelineErrorCode }): Promise<void>;
    releaseStep(input: { idempotencyKey: string; step: PipelineStepName }): Promise<void>;
  };
...
export function listingPipelineJobId(input: ListingPipelineInput): string { return `listing:${input.workspaceId}:${input.draftId}:${input.activeVersionSequence}`; }
...
export async function runListingPipeline(input: ListingPipelineInput, deps: PipelineDependencies, options: PipelineAttemptOptions = {}): Promise<PipelineResult> {
  ...
  try {
    const draft = await deps.withWorkspace(input.workspaceId, async (repos) => {
      ...
      if (claim.claimed) { claimedStep = "started"; await repos.pipelineRuns.recordStep({ idempotencyKey, listingId: input.draftId, activeVersionSequence: input.activeVersionSequence, step: "started" }); claimedStep = null; }
      return listing;
    });
    const source = await deps.withWorkspace(input.workspaceId, async (repos) => ({ assets: await repos.sourceAssets.listForListing(input.draftId), profile: await repos.workspaces.requireProfile() }));
    const extractionClaim = await deps.withWorkspace(input.workspaceId, (repos) => repos.pipelineRuns.claimStep({ idempotencyKey, listingId: input.draftId, activeVersionSequence: input.activeVersionSequence, step: "extracted" }));
    activeStep = "extracted";
    let extraction: ExtractionResult;
    const cachedExtraction = asExtraction(extractionClaim.output);
    if (extractionClaim.claimed) claimedStep = "extracted";
    if (extractionClaim.completed && cachedExtraction) extraction = cachedExtraction;
    else if (!extractionClaim.claimed) throw new Error("extraction step is already running");
    else {
      extraction = await deps.ai.extract({ assets: await deps.assetInputs(source.assets), note: draft.note });
      await deps.withWorkspace(input.workspaceId, async (repos) => { await repos.aiRuns.append(aiRunFrom("extract", extraction.usage, input)); await repos.pipelineRuns.recordStep({ idempotencyKey, listingId: input.draftId, activeVersionSequence: input.activeVersionSequence, step: "extracted", output: extraction }); claimedStep = null; });
    }
    if (extraction.missingFields.length > 0) {
      const result: PipelineResult = { status: "needs_info", versionId: null };
      await deps.withWorkspace(input.workspaceId, async (repos) => { await repos.listings.complete(input.draftId, { ...result, idempotencyKey }, context(input), repos.audit); await repos.pipelineRuns.complete({ idempotencyKey, listingId: input.draftId, activeVersionSequence: input.activeVersionSequence, status: result.status, versionId: result.versionId }); });
      return result;
    }
    const generationClaim = await deps.withWorkspace(input.workspaceId, (repos) => repos.pipelineRuns.claimStep({ idempotencyKey, listingId: input.draftId, activeVersionSequence: input.activeVersionSequence, step: "generated" }));
    activeStep = "generated";
    const cachedGenerated = asGenerated(generationClaim.output);
    if (generationClaim.claimed) claimedStep = "generated";
    ...
    if (!generationClaim.claimed) throw new Error("generation step is already running");
    const generation = await deps.ai.generate({ facts: extraction.facts, evidence: extraction.evidence, profile: source.profile, imageAssetIds: source.assets.filter((asset) => asset.mimeType.startsWith("image/")).map((asset) => asset.id) });
    ...
    const result = await deps.withWorkspace(input.workspaceId, async (repos) => {
      ...
      await repos.pipelineRuns.recordStep({ idempotencyKey, listingId: input.draftId, activeVersionSequence: input.activeVersionSequence, step: "generated", output: { versionId: version.id } }); claimedStep = null;
      ...
      await repos.pipelineRuns.complete({ idempotencyKey, listingId: input.draftId, activeVersionSequence: input.activeVersionSequence, status: completedResult.status, versionId: completedResult.versionId });
