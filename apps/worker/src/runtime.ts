...
import { OpenAIListingProvider } from "@wukong/ai";
...
function mapRepositories(repositories: WorkspaceRepositories, assetStore: AssetStore): PipelineRepositories {
  return {
    ...
    aiRuns: {
      async append(run) {
        await repositories.aiRuns.append({ listingId: run.draftId, ...run, provider: "openai", input: { task: run.task }, output: {}, status: "succeeded" });
      },
    },
    sourceAssets: {
...
export async function startListingPipelineWorker(config: ListingWorkerRuntimeConfig = {}): Promise<ListingWorkerRuntime> {
  ...
  const provider = config.providerFactory ? config.providerFactory() : (() => { required(undefined, "OPENAI_API_KEY"); return new OpenAIListingProvider(); })();
  ...
  const dependencies: PipelineDependencies = {
    async withWorkspace<T>(workspaceId: string, work: (repositories: PipelineRepositories) => Promise<T>) {
      return database.forWorkspace(workspaceId, async (repositories) => work(mapRepositories(repositories, assetStore)));
    },
