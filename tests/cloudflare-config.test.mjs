...
test("removes the Railway and Redis/BullMQ runtime surface", () => {
  ...
  assert.equal(
    ...
    "node --test tests/ci-workflow.test.mjs tests/cloudflare-config.test.mjs tests/runtime-doctor.test.mjs tests/runtime-env-manifest.test.mjs && turbo run test",
  );
});
