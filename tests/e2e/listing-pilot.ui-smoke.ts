...
test.beforeEach(() => {
  test.skip(
    ...
    "Set PLAYWRIGHT_E2E=1 to run the deterministic pilot server.",
  );
});

test("Opak operator completes intake, evidence review, approval, CSV, and mock delivery", async ({
  page,
}) => {
  ...
  await page.getByLabel("產品圖片或文件").setInputFiles({
    ...
    buffer: Buffer.from(
      '<svg xmlns="http://www.w3.org/2000/svg"><text>Opak Cellar</text></svg>',
    ),
  });
  await page
    .getByLabel("補充資料")
    .fill(
      "Opak Cellar Riesling 2024, Germany, Mosel, Riesling, 750ml, 12.5% ABV, SKU OPAK-DEMO-001, HK$288",
    );
  ...
  const beforeApproval = await page.request.post(
    "/api/listings/draft-1/deliver",
    { data: { method: "csv" } },
  );
  ...
  await page
    .getByLabel("英文標題")
    .fill("Opak Cellar Riesling 2024 — reviewed");
  ...
  const csvResponsePromise = page.waitForResponse(
    (response) =>
      response.url().endsWith("/api/listings/draft-1/deliver") &&
