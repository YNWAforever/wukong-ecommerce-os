...
function labels(locale: Locale) {
  ...
  return {
    progress: zh ? "酒類資料處理" : "Wine enrichment",
    sections: zh ? "雙語段落" : "Bilingual sections",
    intro: zh ? /介紹.*繁體中文/ : /Introduction.*English/,
    serving: zh ? /飲用建議.*繁體中文/ : /Serving.*English/,
    initialIntro: zh
      ? "瓶裝容量為 750 毫升。"
    ...
    savedIntro: zh ? "操作員已儲存的介紹。" : "Operator saved introduction.",
    discard: zh ? "放棄修改並載入最新資料" : "Discard edits and load latest",
    save: zh ? "儲存段落" : "Save sections",
    diff: zh ? "採納前比較差異" : "Compare differences before adoption",
    adopt: zh ? "採納所選差異" : "Adopt selected differences",
    identity: zh ? "核對產品身份" : "Confirm product identity",
    confirm: zh ? "確認所選身份" : "Confirm selected identity",
    inspection: zh ? /僅供檢視/ : /Inspection only/,
    unknown: zh ? /未知年份/ : /Unknown vintage/,
  };
}
    ...
    JSON.stringify(
      {
        ...
        syntheticOnly: true,
        locale: (await page.context().cookies()).find(
...
for (const locale of ["en", "zh-Hant"] as const) {
  test(`synthetic wine upload persists through real Web, S3 and Queue (${locale})`, async ({
    page,
  }, info) => {
    test.skip(
    ...
    await page.screenshot({
      path: info.outputPath(`wine-upload-${locale}.png`),
      fullPage: true,
    });
    ...
    await expect(
      page.getByRole("heading", { name: copy.progress, exact: true }),
    ).toBeVisible();
    await page.screenshot({
      path: info.outputPath(`wine-progress-${locale}.png`),
      fullPage: true,
    });
    await page.goto("/listings");
    await page.goto(`/listings/${listingId}`);
    await expect(
      page.getByRole("heading", { name: copy.sections }),
    ).toBeVisible({ timeout: 60000 });
    await expect(page.getByLabel(copy.intro)).toHaveValue(copy.initialIntro, {
      timeout: 60000,
    });
    await page.screenshot({
      path: info.outputPath(`wine-review-${locale}.png`),
      fullPage: true,
    });
    ...
    await page
      .getByRole("heading", { name: copy.sections })
      .locator("..")
      .screenshot({ path: info.outputPath(`wine-paragraphs-${locale}.png`) });
    const intro = page.getByLabel(copy.intro);
    await intro.fill(copy.draftIntro);
    page.once("dialog", (d) => d.accept());
    await page.goBack();
    await expect(page).toHaveURL(/\/listings$/);
    await page.goForward();
    await expect(intro).toHaveValue(copy.draftIntro);
    page.once("dialog", (d) => d.dismiss());
    await page.getByRole("button", { name: copy.discard }).click();
    await expect(intro).toHaveValue(copy.draftIntro);
    page.once("dialog", (d) => d.accept());
    await page.getByRole("button", { name: copy.discard }).click();
    await expect(intro).toHaveValue(copy.initialIntro);
    await intro.fill(copy.savedIntro);
    ...
    await page.reload();
    await expect(intro).toHaveValue(copy.savedIntro);
    ...
    await expect(page.getByRole("heading", { name: copy.diff })).toBeVisible({
      timeout: 60000,
    });
    await expect(
    ...
    await page.screenshot({
      path: info.outputPath(`wine-evidence-diff-${locale}.png`),
      fullPage: true,
    });
    const adopted = page.waitForResponse(
      (r) => r.request().method() === "POST" && r.url().endsWith("/adopt"),
    );
    await page.getByRole("button", { name: copy.adopt }).click();
    expect((await adopted).status()).toBe(200);
    await expect(intro).toHaveValue(copy.savedIntro);
    await expect(page.getByLabel(copy.serving)).toHaveValue(
      locale === "zh-Hant" ? "750 ml" : "Bottle capacity: 750 ml.",
    );
    ...
    await expect(page.getByLabel(copy.intro)).toHaveValue(copy.savedIntro);
    await page
      .getByRole("heading", { name: copy.sections })
      .locator("..")
      .screenshot({
        path: info.outputPath(`wine-saved-paragraphs-${locale}.png`),
      });
    await page.screenshot({
      path: info.outputPath(`wine-review-final-${locale}.png`),
      fullPage: true,
    });
    expect(
      (
...
for (const scenario of [
  ...
  for (const locale of (scenario === "ambiguous"
    ? ["en", "zh-Hant"]
    : ["en"]) as Locale[]) {
    test(`actual synthetic HTTP journey: ${scenario} (${locale})`, async ({
      page,
    }, info) => {
      test.skip(
        process.env.WUKONG_WINE_E2E !== "1",
      ...
      if (scenario === "empty") {
        ...
        await expect(
          page.getByRole("region", { name: copy.progress }).getByRole("status"),
        ).toHaveText("Failed");
        ...
        expect(
          view.wineProgress.issues.some(
            (issue: { code: string }) =>
              issue.code === "trusted_source_conflict",
          ),
        ).toBe(true);
        ...
        const historical = page.getByRole("radio", { name: copy.inspection });
        ...
        await expect(historical).toHaveAccessibleName(copy.unknown);
        await expect(
          page
            .getByRole("group", { name: copy.identity })
        ...
        await page.getByRole("group", { name: copy.identity }).screenshot({
          path: info.outputPath(`wine-ambiguity-${locale}.png`),
        });
        const confirmed = page.waitForResponse(
          (r) =>
            r.request().method() === "POST" && r.url().endsWith("/identity"),
        );
      ...
      const provider = await fetch("http://127.0.0.1:49221/evidence").then(
        (r) => r.json(),
      );
      expect(
      ...
      await page.getByRole("region", { name: copy.progress }).screenshot({
        path: info.outputPath(`wine-${scenario}-${locale}.png`),
      });
    });
  }
}
