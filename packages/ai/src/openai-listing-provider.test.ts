...
const groundingNote =
  "SKU OPAK-DEMO-001; producer Demo Estate; product type wine; country Germany; region Mosel; vintage 2024; grapes Riesling; volume 750 ml; ABV 12.5%; pack quantity 1; price HK$288.";
...
const listingFixture = {
  ...
  title: {
    ...
    "zh-Hant": "Demo Estate Riesling 2024",
  },
...
describe("OpenAIListingProvider", () => {
      ...
      assets: [
        {
          id: "asset_image",
          mimeType: "image/png",
          readUrl: "https://assets.example/image.png",
        },
        {
          id: "asset_pdf",
          mimeType: "application/pdf",
          readUrl: "https://assets.example/sheet.pdf",
        },
      ],
      note: groundingNote,
    });
  ...
  it("uses the environment model by default and explicit config takes precedence", async () => {
    ...
    try {
      ...
      await new OpenAIListingProvider(fromEnv.client).extract({
        assets: [],
        note: groundingNote,
      });
      expect(fromEnv.parse).toHaveBeenCalledWith(
        expect.objectContaining({ model: "gpt-5.6-luna" }),
        expect.objectContaining({ signal: expect.any(AbortSignal) }),
      );

      const explicit = fakeClient(extractionResponse());
      await new OpenAIListingProvider(explicit.client, {
        model: "gpt-5.6-terra",
      }).extract({ assets: [], note: groundingNote });
      expect(explicit.parse).toHaveBeenCalledWith(
  ...
  it.each(["", " unsafe model ", "../model", "model?secret=1"])(
    "rejects unsafe model config %j",
    (model) => {
      expect(() => new OpenAIListingProvider(undefined, { model })).toThrow(
        /model/i,
      );
    },
  );
  ...
  it("uses the default bounded deadline for both initial and repair requests", async () => {
    ...
    await new OpenAIListingProvider(client).extract({
      assets: [],
      note: groundingNote,
    });
  ...
  it("accepts inclusive provider timeout bounds and rejects unsafe deadlines", () => {
    expect(
      () => new OpenAIListingProvider(undefined, { timeoutMs: 1_000 }),
    ).not.toThrow();
    expect(
      () => new OpenAIListingProvider(undefined, { timeoutMs: 600_000 }),
    ).not.toThrow();
    for (const timeoutMs of [
      ...
      expect(() => new OpenAIListingProvider(undefined, { timeoutMs })).toThrow(
        /timeout/i,
      );
    }
  });
  ...
  it.each([
    ...
    null,
    ["gpt-5.6-terra"],
    { toString: () => "gpt-5.6-terra" },
  ])("rejects non-string model config %j", (model) => {
    expect(
      () => new OpenAIListingProvider(undefined, { model: model as never }),
    ).toThrow(/model/i);
  });
  ...
  it.each([
    [
      "unsupported MIME",
      {
        id: "asset",
        mimeType: "text/plain",
        readUrl: "https://assets.example/a.txt",
      },
    ],
    [
      "non-HTTPS URL",
      {
        id: "asset",
        mimeType: "image/png",
        readUrl: "http://assets.example/a.png",
      },
    ],
    [
      "local URL",
  ...
  it("applies the documented long-context pricing multipliers deterministically", async () => {
    ...
    const result = await new OpenAIListingProvider(client).extract({
      assets: [],
      note: groundingNote,
    });
  ...
  it.each([
    ...
    { inputUsdPerMillion: 2.5, outputUsdPerMillion: Number.NaN },
    {
      ...
      longContextThresholdTokens: -1,
    },
    {
      inputUsdPerMillion: 2.5,
      outputUsdPerMillion: 15,
      longContextThresholdTokens: Number.NaN,
    },
    {
      inputUsdPerMillion: 2.5,
      outputUsdPerMillion: 15,
    ...
    },
    {
      inputUsdPerMillion: 2.5,
      outputUsdPerMillion: 15,
      longContextOutputMultiplier: Number.NaN,
    },
  ])("rejects invalid pricing config %#", (pricing) => {
    expect(() => new OpenAIListingProvider(undefined, { pricing })).toThrow(
      /pricing/i,
    );
  });
  ...
  it.each([
    ...
    "keeps latency finite and non-negative for clocks %s -> %s",
    async (start, end) => {
  ...
  it("rejects extracted facts with empty or irrelevant evidence", async () => {
    ...
    await expect(
      new OpenAIListingProvider(empty.client).extract({
        assets: [],
        note: groundingNote,
      }),
    ).rejects.toBeInstanceOf(ProviderOutputError);

    const irrelevant = fakeClient(
      extractionResponse({
        output_parsed: {
          ...extractionFixture,
          evidence: evidence.map((item) =>
            item.field === "producer" ? { ...item, excerpt: "Germany" } : item,
          ),
        },
      }),
    );
    await expect(
      new OpenAIListingProvider(irrelevant.client).extract({
        assets: [],
        note: groundingNote,
      }),
    ).rejects.toBeInstanceOf(ProviderOutputError);
  ...
  it("allows the schema-default pack quantity without source evidence", async () => {
    ...
    await expect(
      ...
      facts: expect.objectContaining({ packQuantity: 1 }),
    });
  });
      ...
      extractionResponse({
        output_parsed: {
          ...
          evidence: evidence.filter(
            (item) =>
              item.field !== "producer" && item.field !== "packQuantity",
          ),
        },
      }),
    );
  ...
  it("rejects substring evidence collisions while accepting exact multiword, numeric, and source claims", async () => {
    const collisions = [
      ...
      },
      {
        ...
        evidence: evidence.map((item) =>
          item.field === "country" ? { ...item, excerpt: "Malian" } : item,
        ),
      },
      {
      ...
      extractionResponse({
        output_parsed: {
          facts: {
            ...facts,
            criticScores: [
              { source: "Wine Advocate", score: "95", evidenceId: "note" },
            ],
  ...
  it("does not construct the default client until the first provider call", () => {
    ...
    expect(
      () => new OpenAIListingProvider(undefined, { clientFactory }),
    ).not.toThrow();
  ...
  it("returns a typed output error after the single repair is exhausted", async () => {
    ...
    await expect(
      new OpenAIListingProvider(client).extract({ assets: [], note: null }),
    ).rejects.toBeInstanceOf(ProviderOutputError);
    expect(parse).toHaveBeenCalledTimes(2);
  });
  ...
  it("does not repair refusals or arbitrary API failures and does not leak provider messages", async () => {
    const refusal = fakeClient({
      ...
      output: [
        {
          type: "message",
          content: [{ type: "refusal", refusal: "private policy detail" }],
        },
      ],
    });
    await expect(
      new OpenAIListingProvider(refusal.client).extract({
        assets: [],
        note: null,
      }),
  ...
  it("preserves model-authored grounded copy after validating protected fields", async () => {
    const modelListing = {
      ...listingFixture,
      title: {
        en: "Model-authored grounded title",
        "zh-Hant": "Model-authored grounded Traditional Chinese title",
      },
      description: {
        en: "Model-authored grounded English copy",
        "zh-Hant": "Model-authored grounded Traditional Chinese copy",
      },
    };
    const { client, parse } = fakeClient({
      output_parsed: { listing: modelListing },
      usage: undefined,
      output: [],
    });
    const result = await new OpenAIListingProvider(client, {
  ...
  it("rejects generated protected-fact mutations", async () => {
    ...
    await expect(
      new OpenAIListingProvider(changed.client).generate({
        facts,
        evidence,
        profile,
        imageAssetIds: ["asset_image"],
  ...
  it("rejects generated image identity or order mutations", async () => {
    const changedIdentity = fakeClient({
      output_parsed: {
        listing: { ...listingFixture, imageAssetIds: ["asset_unknown"] },
      },
      output: [],
    });
    await expect(
      new OpenAIListingProvider(changedIdentity.client).generate({
        facts,
        evidence,
        profile,
        imageAssetIds: ["asset_image"],
    ...
    const reordered = fakeClient({
      output_parsed: {
        listing: {
          ...listingFixture,
          imageAssetIds: ["asset_second", "asset_image"],
        },
      },
      output: [],
    });
    await expect(
      new OpenAIListingProvider(reordered.client).generate({
        facts,
        evidence,
        profile,
        imageAssetIds: ["asset_image", "asset_second"],
      }),
    ).rejects.toBeInstanceOf(ProviderOutputError);
  });

  it("rejects unsupported generation input evidence", async () => {
    const unsupported = fakeClient({
      output_parsed: { listing: listingFixture },
      output: [],
    });

    await expect(
      new OpenAIListingProvider(unsupported.client).generate({
        facts,
