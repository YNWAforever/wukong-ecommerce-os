...
export type BulkFormProfile = {
  ...
  rows: number;
  issues: { total: number; errors: number; warnings: number; byCode: Record<string, number> };
  gaps: Record<string, number>;
  inventory: { zeroStock: number; unlimited: number; hiddenOnline: number; retailVisible: number };
...
export function profileBulkForm(result: BulkFormParseResult): BulkFormProfile {
  ...
  const inventory = { zeroStock: 0, unlimited: 0, hiddenOnline: 0, retailVisible: 0 };
  const pricing = { onSale: 0, withCost: 0, missingPrice: 0 };

  for (const row of result.rows as readonly BulkFormProductRow[]) {
    for (const [gap, present] of Object.entries(row.gaps)) if (present) tally(gaps, gap);
    ...
    if (sale !== null && regular !== null && sale > 0 && sale < regular) pricing.onSale += 1;
...
export function main(args: readonly string[] = process.argv.slice(2)): number {
  const path = args[0];
  if (path === undefined) {
    console.error("usage: pnpm --filter @wukong/shopline bulk-form:profile <bulk-update-form.xlsx>");
    return 1;
  }

  try {
    const result = parseBulkForm(readSheet(path));
    console.info(JSON.stringify({ event: "bulk_form_profiled", ...profileBulkForm(result) }));
