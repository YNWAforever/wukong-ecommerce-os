...
export type WorkspaceSeedStore = {
...
export type OpakSeedStore = WorkspaceSeedStore;
...
export type OpakSeedResult = WorkspaceSeedResult;

export async function seedWorkspace(
  store: WorkspaceSeedStore,
  config: WorkspaceSeedConfig,
  operatorEmail: string,
): Promise<WorkspaceSeedResult> {
  const email = parseEmail(operatorEmail);
  const workspaceId = await store.upsertWorkspace({
    id: config.workspaceId,
    name: config.profile.name,
    profile: config.profile,
  });
  const userId = await store.upsertUser({ id: config.userId, email });
  await store.upsertMembership({ workspaceId, userId, role: "operator" });
  await store.upsertProfile({ workspaceId, profile: config.profile });
  await store.upsertPromptVersion({ workspaceId, ...config.prompt });
  ...
  return { workspaceId, userId, email, promptVersion: config.prompt.version };
}

export async function seedOpak(
  store: OpakSeedStore,
  operatorEmail: string,
): Promise<OpakSeedResult> {
  return seedWorkspace(
    store,
    {
      workspaceId: OPAK_WORKSPACE_ID,
      userId: OPAK_OPERATOR_ID,
      profile: opakProfile,
      prompt: {
        key: OPAK_PROMPT_KEY,
        version: OPAK_PROMPT_VERSION,
        template: opakPromptTemplate,
...
export function createOpakSeedStore(db: AuthDatabase): OpakSeedStore {
  return {
    async upsertWorkspace(input) {
      await db
        .insert(workspaces)
        .values({ id: input.id, name: input.name, profile: input.profile })
        .onConflictDoUpdate({
          target: workspaces.id,
          set: { name: input.name, profile: input.profile },
        });
      return input.id;
    },
    ...
    async upsertPromptVersion(input) {
      await db
        ...
        .onConflictDoUpdate({
          target: [
            promptVersions.workspaceId,
            promptVersions.key,
            promptVersions.version,
          ],
          set: { template: input.template, model: input.model },
        });
    },
...
export async function seedOpakFromEnv(): Promise<OpakSeedResult> {
  ...
  try {
    return await seedOpak(createOpakSeedStore(db), email);
  } finally {
    await db.close();
  }
}
