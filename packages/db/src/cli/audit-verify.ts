...
function requiredSequenceMissing(actions: readonly string[]): string[] {
  ...
  for (const required of REQUIRED_AUDIT_SEQUENCE) {
    const found = actions.findIndex((action, index) => index >= cursor && action === required);
...
export async function verifyAudit(input: AuditVerifyInput): Promise<AuditVerificationResult> {
  if (!input.workspaceId.trim() || !input.draftId.trim()) throw new Error("workspace and draft are required");
  const client = postgres(input.url, { connect_timeout: 10, max: 1, prepare: false });
  try {
...
export function parseAuditVerifyArgs(args: readonly string[], env: NodeJS.ProcessEnv = process.env): AuditVerifyInput {
  const workspaceId = readArg(args, "--workspace") ?? env.OPAK_WORKSPACE_ID ?? "ws_opak";
  const draftId = readArg(args, "--draft");
  const url = env.DATABASE_URL;
  if (!draftId) throw new Error("usage: audit:verify --workspace <workspace-id> --draft <draft-id>");
...
if (process.argv[1]?.endsWith("audit-verify.ts") || process.argv[1]?.endsWith("audit-verify.js")) {
