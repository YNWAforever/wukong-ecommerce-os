...
describe("listing pipeline run repository", () => {
  ...
  it("hides a pipeline recovery record from another workspace", async () => {
    const owner = await forWorkspace(
      database,
      "ws_pipeline_owner",
      async (repos) => {
        const listing = await repos.listings.create({ target: "shopline" });
    ...
    await expect(
      forWorkspace(database, "ws_pipeline_other", (repos) =>
        repos.pipelineRuns.getCompleted(owner),
      ),
    ).resolves.toBeNull();
  });
  ...
  it("claims each step once, stores recovery output, and records terminal failure codes", async () => {
    const snapshot = await forWorkspace(
      ...
      async (repos) => {
        const listing = await repos.listings.create({ target: "shopline" });
        const input = {
          idempotencyKey: `listing:ws_pipeline_claim:${listing.id}:0`,
          listingId: listing.id,
          activeVersionSequence: 0,
        };
        const first = await repos.pipelineRuns.claimStep({
          ...input,
          step: "extracted",
        });
        await repos.pipelineRuns.recordStep({
        ...
        const second = await repos.pipelineRuns.claimStep({
          ...input,
          step: "extracted",
        });
        ...
        return {
          first,
          second,
          state: await repos.pipelineRuns.getState(input.idempotencyKey),
        };
      },
    );
  ...
  it("does not let a stale worker release a reclaimed step owned by a newer worker", async () => {
    const initial = await forWorkspace(
      database,
      "ws_pipeline_lease",
      async (repos) => {
        const listing = await repos.listings.create({ target: "shopline" });
    ...
    const workerB = await forWorkspace(database, "ws_pipeline_lease", (repos) =>
      repos.pipelineRuns.claimStep(initial.input),
    );
    ...
    const afterStaleRelease = await forWorkspace(
      database,
      "ws_pipeline_lease",
      async (repos) => {
    ...
    expect(afterStaleRelease?.steps.get("extracted")).toMatchObject({
      state: "running",
    });

    const afterOwnerRelease = await forWorkspace(
      database,
      "ws_pipeline_lease",
      async (repos) => {
        await repos.pipelineRuns.releaseStep({
  ...
  it("lists workspace pipeline runs newest first, isolated per workspace, with limit bounds enforced", async () => {
    ...
    const listingIds = await forWorkspace(
      database,
      "ws_pipeline_list",
      async (repos) => {
        const ids: string[] = [];
    ...
    for (const [index, listingId] of listingIds.entries()) {
      const backdated = new Date(
        Date.now() - (listingIds.length - index) * 60_000,
      );
      await admin.unsafe(
    ...
    const listed = await forWorkspace(database, "ws_pipeline_list", (repos) =>
      repos.pipelineRuns.listForWorkspace(),
    );
    expect(listed.map((run) => run.listingId)).toEqual(
      [...listingIds].reverse(),
    );
    ...
    await expect(
      forWorkspace(database, "ws_pipeline_list", (repos) =>
        repos.pipelineRuns.listForWorkspace(0),
      ),
    ).rejects.toThrow(/limit must be between 1 and 100/i);
    await expect(
      forWorkspace(database, "ws_pipeline_list", (repos) =>
        repos.pipelineRuns.listForWorkspace(101),
      ),
    ).rejects.toThrow(/limit must be between 1 and 100/i);
  });
  ...
  it("breaks a created_at tie deterministically by id when several pipeline runs share one transaction's now()", async () => {
    ...
    const { listingIds, listed } = await forWorkspace(
      database,
      "ws_pipeline_tie",
      async (repos) => {
        const ids: string[] = [];
        ...
        return {
          listingIds: ids,
          listed: await repos.pipelineRuns.listForWorkspace(),
        };
      },
    );
