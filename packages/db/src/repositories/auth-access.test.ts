...
describe("auth access repository", () => {
  ...
  const migrationDatabase = createDatabase(adminUrl, { migrationUrl: adminUrl });
  ...
  beforeEach(async () => {
    ...
    await db.insert(workspaces).values({ id: workspaceId, name: "Auth access", profile: {} });
  ...
  it.each(["pending", "accepted"])("normalizes email and accepts a %s invite", async (status) => {
  ...
  it("increments normalized consecutive failures and locks on the fifth for 15 minutes", async () => {
    await expect(repository.recordPasswordFailure(" Admin@Example.com ", now)).resolves.toEqual({
      failedAttempts: 1,
      lockedUntil: null,
    });
    ...
    expect(fifthFailure.lockedUntil?.toISOString()).toBe("2026-07-15T00:15:00.000Z");
    await expect(repository.getPasswordGuard(" ADMIN@example.com ", now)).resolves.toEqual(fifthFailure);
  });
  ...
  it("resets an expired lock before recording the next failure", async () => {
    ...
    await expect(repository.getPasswordGuard(email, now)).resolves.toEqual({ failedAttempts: 0, lockedUntil: null });
    await expect(repository.recordPasswordFailure(email, now)).resolves.toEqual({ failedAttempts: 1, lockedUntil: null });
  });
  ...
  it("atomically completes enrollment for an eligible invite", async () => {
    await db.insert(workspaceInvites).values({ workspaceId, email, role: "operator", status: "pending" });
    await repository.recordPasswordFailure(email, now);
    await repository.completeEnrollment(userId, " Admin@Example.com ");

    const [user] = await db.select({ verified: users.emailVerified }).from(users).where(eq(users.id, userId));
    const [invite] = await db.select({ status: workspaceInvites.status }).from(workspaceInvites);
    ...
    expect(audit).toEqual({ email, userId, outcome: "success", reason: "password_enrollment_completed" });
  });

  it("does not partially enroll a user without an eligible invite", async () => {
    await db.insert(workspaceInvites).values({ workspaceId, email, role: "operator", status: "revoked" });
    await expect(repository.completeEnrollment(userId, email)).rejects.toThrow(/eligible invite/i);
    const [user] = await db.select({ verified: users.emailVerified }).from(users).where(eq(users.id, userId));
  ...
  it("revokes all sessions for only the requested user", async () => {
    await db.insert(authSessions).values([
      { id: "session_auth_access_1", userId, token: "token-1", expiresAt: new Date("2026-07-16T00:00:00Z") },
      { id: "session_auth_access_2", userId, token: "token-2", expiresAt: new Date("2026-07-16T00:00:00Z") },
      { id: "session_auth_access_other", userId: otherUserId, token: "token-other", expiresAt: new Date("2026-07-16T00:00:00Z") },
    ]);
    await repository.revokeUserSessions(userId);
    const remaining = await db.select({ userId: authSessions.userId }).from(authSessions);
  ...
  it("writes normalized, secret-free authentication audit events", async () => {
    ...
    expect(audit).toEqual({ email, userId, outcome: "failure", reason: "invalid_credentials" });
  });
});
