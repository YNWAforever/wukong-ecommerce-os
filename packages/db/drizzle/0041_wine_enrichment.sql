...
CREATE TABLE IF NOT EXISTS wine_evidence (
 workspace_id text NOT NULL REFERENCES workspaces(id),run_id uuid NOT NULL,source_id uuid NOT NULL,payload jsonb NOT NULL CHECK(jsonb_typeof(payload)='object' AND payload ? 'schemaVersion' AND payload->>'schemaVersion'='1'),created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS wine_section_snapshots (
 workspace_id text NOT NULL REFERENCES workspaces(id),run_id uuid NOT NULL,listing_id uuid NOT NULL,version_id uuid NOT NULL,payload jsonb NOT NULL CHECK(jsonb_typeof(payload)='object' AND payload ? 'schemaVersion' AND payload->>'schemaVersion'='1'),created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS wine_source_authorities (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(),workspace_id text NOT NULL REFERENCES workspaces(id),reviewer_id text NOT NULL,payload jsonb NOT NULL CHECK(jsonb_typeof(payload)='object' AND payload ? 'schemaVersion' AND payload->>'schemaVersion'='1'),created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS wine_trusted_contexts (
 workspace_id text NOT NULL REFERENCES workspaces(id),run_id uuid NOT NULL,context_key text NOT NULL,input_digest text NOT NULL,payload jsonb NOT NULL CHECK(jsonb_typeof(payload)='object' AND payload ? 'schemaVersion' AND payload->>'schemaVersion'='1'),created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS search_budget_reservations (
 ...
 CONSTRAINT search_budget_units_check CHECK((state IN ('held','unknown') AND settled_credits IS NULL) OR (state='settled' AND settled_credits BETWEEN 0 AND reserved_credits))
);
CREATE TABLE IF NOT EXISTS wine_search_calls (
 ...
 CONSTRAINT wine_calls_units_check CHECK((status IN ('started','unknown') AND credits IS NULL) OR (status IN ('succeeded','failed') AND credits BETWEEN 0 AND maximum_credits)),
