...
## Final review fix: bind comparison to the actual Default worksheet

Reproduced P2 with actual valid synthetic multi-worksheet XLSX: first tab Default has rId2 -> worksheets/sheet2.xml with changed SKU; Archive has rId1 -> sheet1.xml matching delivered artifact. Previous independent sheet-name and numeric-sheet readers persisted matches_compared_fields. RED command: corepack.cmd pnpm@11.7.0 --filter @wukong/web exec vitest run lib/fresh-export-verification.test.ts -t 'actual Default'; expected differences_found, received matches_compared_fields.

Added comparison-only readDefaultBulkFormSheet in packages/shopline/src/bulk-form-xlsx.ts. It resolves package officeDocument to canonical workbook metadata, finds the unique declared Default name and r:id, validates that exact relationship as an internal worksheet, and reads that target. It rejects missing/duplicate relationship identities, duplicate Default declarations, unsafe/external/missing targets, duplicate ZIP parts, ambiguous attributes, unsupported namespace metadata and unsupported shared-string bindings. Canonical relative or absolute internal targets and canonical shared strings are supported. It never falls back to numeric sheet order. Legacy readBulkFormSheet/readBulkFormSheetName behavior is unchanged; their shared cell decoding was extracted without changes. Both immutable delivered and supplied bytes use the new relationship-bound comparison reader. Invalid/unsupported workbook layouts return existing safe comparison_workbook_invalid400 before persistence; no wire/UI/database changes.

GREEN: actual reordered workbook now persists differences_found and SKU observed CHANGED. Four service negatives (missing/duplicate/external/traversal) prove ensure is never called for invalid evidence. Combined service+route suites42/42. Full Shopline suite248/248 in12files (20 new relationship cases), including legacy reader regressions and explicit same-bytes legacy-selection parity. Canonical shared-string positive case and namespace/unbound/duplicate-attribute negatives also pass. Shopline build passed. Web typecheck initially caught a too-narrow test fixture literal array type; annotated the changed matrix as string[][] and reran.

All tests synthetic. No DB/service lifecycle mutation performed. This follow-up is local only and leaves root-owned final browser acceptance/combined checks to root.

Final web typecheck GREEN after fixture type correction. Final formatting and git diff --check passed.
